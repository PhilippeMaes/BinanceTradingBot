...
@SpringBootTest
public class RenkoTest2 {

    private static final String BASE = "BTC";
    private static final String MARKET = "ICX";
    private static final Double START_BASE = 1000.0;
    private static final Integer ATR_LENGTH = 14;
...
//            ATR = range.stream().mapToDouble(c -> Double.valueOf(c.getHigh())).max().getAsDouble() / range.stream().mapToDouble(c -> Double.valueOf(c.getLow())).min().getAsDouble() - 1.0;
            ATR = 0.025;

            double currentPrice = Double.valueOf(candlestick.getClose());
            double percentageDiff = currentPrice / lastBlockPrice;
            int blocks = (int) ((percentageDiff - 1.0) / ATR);

            lastBlockPrice = (1.0 + ATR * blocks) * lastBlockPrice;
